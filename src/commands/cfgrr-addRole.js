...
module.exports = {    
    ...
    async execute(message, args, config, rrConfig) {
        ...
        if(rrConfig.nRoles == 1){
            if(rol.id == rrConfig.rol1.id || rol.emoji == rrConfig.rol1.emoji ) return message.channel.send(`You can't neither repeated rol or multiples roles with the same emoji !!`);
            replace(__dirname + "/../configs/rrConfig.json", "rol2", rol);
        }
        if(rrConfig.nRoles == 2){
            if(rol.id == rrConfig.rol1.id || rol.emoji == rrConfig.rol1.emoji || rol.id == rrConfig.rol2.id || rol.emoji == rrConfig.rol2.emoji) return message.channel.send(`You can't neither repeated rol or multiples roles with the same emoji !!`);

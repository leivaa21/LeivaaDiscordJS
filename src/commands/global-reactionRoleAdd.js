    ...
    async execute(reaction, user, rrConfig) {
        ...
        if(rrConfig.nRoles > 0){  
            if (reaction._emoji.name == rrConfig.rol1.emoji)
                await reaction.message.guild.members.cache.get(user.id).roles.add(rrConfig.rol1.id);
        }
        if(rrConfig.nRoles > 1){
            if (reaction._emoji.name == rrConfig.rol2.emoji)
                await reaction.message.guild.members.cache.get(user.id).roles.add(rrConfig.rol2.id);
               
        }
        if(rrConfig.nRoles > 2){
            if (reaction._emoji.name == rrConfig.rol3.emoji)
                await reaction.message.guild.members.cache.get(user.id).roles.add(rrConfig.rol3.id);
        }    
    }
}